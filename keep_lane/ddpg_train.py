...
def target_thread(sess, online_begin_signal):
    ...
    while True:
        ...
        if begin and memory_pool.capacity_bigger_than(val=30):
            begin = False
...
def online_thread(sess, online_begin_signal):
    ...
    while True:
        ...
        next_action = sess.run(action_target, feed_dict={target_img_state:next_img_state})
        up, q_l = sess.run([online_critic_update, q_loss], feed_dict={reward: current_reward, whether_end: end_flag, target_img_state: next_img_state,
                                                                      target_action: next_action, online_img_state: current_img_state,
                                                                      online_action: current_action, lr:FLAGS.critic_learning_rate})

        ## update the Online Actor π(s) ##
        sess.run([online_actor_update], feed_dict={online_img_state: current_img_state, online_action: current_action,
...
if __name__ == '__main__':
    ...
    q_target, target_critic_vars = target_critic.build_graph(img_state=target_img_state, action=target_action, is_training=True,
    ...
    q_online, online_critic_vars = online_critic.build_graph(img_state=online_img_state, action=online_action, is_training=True,
